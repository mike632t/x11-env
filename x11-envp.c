 ...
 *
 * Creates an X11 window and display a list of the environment vairables in 
 * the window and on standard output before waiting for the user to press a
 * key or click on the window.  
 * 
 * The window can be resized as required to display all the output.
 *
...
int main(int argc, char **argv, char **envp)
{
   ...
   if (h_display) /*   If successful create and display a new window. */
   {
      ...
      for(char ** h_pointer = envp; *h_pointer; h_pointer++) /* Print the environment vairables on the console */
      {
         fprintf(stdout, "%s\n", *h_pointer); 
      }

      while (True) /* Main program event loop */
      {
         ...
         if (x_event.type == Expose) 
         {
            ...
            for(char ** h_pointer = envp; *h_pointer; h_pointer++)
            {
               ...
               XDrawString(h_display, x_application_window,
                  ...
                  i_offset, i_position,
                  *h_pointer, strlen(*h_pointer));
               i_offset += XTextWidth(h_font, *h_pointer, strlen(*h_pointer));
